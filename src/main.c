...
/*   Updated: 2024/08/05 22:34:00 by lelichik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_philo(t_general *info)
{
	...
	while (i < info->count_philo)
	{
		ph = &info->philo[i];
		ph->num_philo = i;
		ph->left_fork = &info->forks[i];
		ph->right_fork = &info->forks[(i + 1) % info->count_philo];
		ph->info = info;
		ph->last_meal = 0;
		ph->count_eat = 0;
...
void	init_general(t_general *info, int ac, char **arg)
{
	...
	pthread_mutex_init(&info->max_eat_mutex, NULL);
}
